...
# can_pkg.sv is always compiled first; everything else under rtl/ and sim/
...
#
#     TOP     top-level testbench module (default: tb_can_bus_top)
...
# ---- sources: can_pkg.sv first, then rtl/ + sim/ ---------------------------
PKG="$ROOT/rtl/can_pkg.sv"
SV_FILES=()
[ -f "$PKG" ] && SV_FILES+=("$PKG")
...
# 2) elaborate
echo ">>> xelab"
xelab "${ELAB_TOPS[@]}" -s "${TOP}_snap" -mt off --debug typical \
