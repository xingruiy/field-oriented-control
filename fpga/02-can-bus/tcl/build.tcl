# ============================================================================
# build.tcl - Vivado non-project flow for can_bus_top (Arty S7-50).
#
#   vivado -mode batch -source tcl/build.tcl
#
# Reads can_pkg.sv first, then globs rtl/*/*.sv (no create_ip phase - the
# design instantiates only BUFG/IBUF primitives directly). Artifacts land
# in build/impl/.
# ============================================================================
...
read_verilog -sv $ROOT/rtl/can_pkg.sv
foreach f [lsort [glob -nocomplain $ROOT/rtl/*/*.sv]] {
    if {[file tail $f] eq "can_pkg.sv"} { continue }
    read_verilog -sv $f
}
