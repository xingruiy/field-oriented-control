# ============================================================================
# build.tcl - Vivado non-project flow for pwm_top (Arty S7-50).
#
#   vivado -mode batch -source tcl/build.tcl
#
# Reads pwm_pkg.sv first, then globs rtl/*/*.sv (no create_ip phase - the
...
read_verilog -sv $ROOT/rtl/pwm_pkg.sv
foreach f [lsort [glob -nocomplain $ROOT/rtl/*/*.sv]] {
    if {[file tail $f] eq "pwm_pkg.sv"} { continue }
    read_verilog -sv $f
}
